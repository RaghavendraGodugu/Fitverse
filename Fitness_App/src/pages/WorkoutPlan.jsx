...
import { Target, User, Check, RefreshCcw, Dumbbell, Scale, CalendarDays, Play, MoreHorizontal } from 'lucide-react';
...
export default function WorkoutPlan() {
  ...
  useEffect(() => {
    ...
    if (saved) {
      ...
      setHasPlan(true);
      // set selected day to current day of week (0 index Monday)
...
Return 7 days starting from Monday to Sunday. If it is a rest day, set "isRest": true, "exercises": [], "focus": "Rest Day", and provide a recovery "analysis".`;
      ...
      if (aiJson.data?.schedule) {
         ...
         const parsedObj = typeof aiJson.data === 'string' ? JSON.parse(aiJson.data) : aiJson.data;
      ...
      // 3. Map structure (Ensure exactly 7 days)
      const finalSchedule = FULL_DAYS.map((fullDayName, idx) => {
         // Try to find by name first, then by index, or default to a rest day
         let dayObj = generatedSchedule.find(d => String(d.dayName).toLowerCase() === fullDayName.toLowerCase());
         if (!dayObj && generatedSchedule[idx]) {
             dayObj = generatedSchedule[idx]; // Fallback to index if AI didn't return proper dayName
         }
         if (!dayObj) {
             dayObj = { dayName: fullDayName, focus: 'Rest Day', isRest: true, exercises: [], analysis: 'Take the day off to rest and recover. Drink plenty of water and prioritize quality sleep to repair.' };
         }

         const mappedExercises = (dayObj.exercises || []).map(exName => {
             const cleanName = typeof exName === 'string' ? exName.toLowerCase() : '';
             const match = allExercises.find(ex => ex.name.toLowerCase().includes(cleanName));
         ...
         return {
            ...dayObj,
            dayName: fullDayName, // Enforce correct day name
            mappedExercises
         };
      });
      ...
      alert("Error generating plan from Gemini or WGER backend.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const activeDay = planData?.schedule?.[selectedDayIdx] || {
    ...
    focus: 'Data Missing',
    isRest: true,
    analysis: 'Please click the top-right refresh icon to cleanly regenerate your complete 7-day schedule.',
    mappedExercises: []
  };
    ...
    >
      <AnimatePresence mode="wait">
        {!hasPlan ? (
          ...
          >
            <div>
              <h1 className="text-3xl md:text-4xl font-bold tracking-tight mb-2">Smart Weekly Workout Plan</h1>
              ...
              <div className="space-y-2">
                ...
                <div className="flex gap-4">
                  ...
                  >
                    ...
                    <option value="Muscular border">Muscular / Hypertrophy</option>
          ...
          >
            ...
            <div className="pt-8 px-6 sm:px-10 pb-10 bg-dark-800 flex flex-col relative z-10 border-b border-white/5">
               <div className="flex justify-between items-center mb-6">
                 <h1 className="text-3xl sm:text-4xl font-black tracking-tight text-transparent bg-clip-text bg-gradient-to-r from-brand-400 to-indigo-400">Your Plan</h1>
                 <button onClick={resetPlan} className="p-2 sm:p-2.5 bg-dark-900 hover:bg-white/5 border border-white/5 rounded-xl transition-all shadow-sm">
                    <RefreshCcw className="w-5 h-5 text-zinc-400 hover:text-white" />
                 </button>
               </div>
               
               <p className="uppercase text-xs tracking-widest font-bold mb-8 text-brand-400">{planData.metadata.goal} • WEEK 1</p>
               ...
               <div className="flex justify-between items-center w-full max-w-md mx-auto mb-2 bg-dark-900 border border-white/5 p-2 rounded-[24px]">
                 {DAYS_OF_WEEK.map((d, i) => (
                    <div key={i} className="flex flex-col items-center gap-1 cursor-pointer group p-1" onClick={() => setSelectedDayIdx(i)}>
                       <div className={cn(
                       ...
                       <div className="h-1.5 w-1.5 shrink-0 flex justify-center">
                         {selectedDayIdx !== i && planData.schedule[i]?.isRest === false && <div className="w-1.5 h-1.5 bg-brand-500/50 rounded-full" />}
                       </div>
                    </div>
                 ))}
               </div>
            ...
            {/* Content Section matches design */}
                  ...
                  >
                     <div className="flex items-center gap-4 mb-5">
                        ...
                        <div className="flex-1">
                           <h2 className="text-2xl font-black text-white tracking-tight">{activeDay.focus}</h2>
                           <p className="text-zinc-400 font-medium">{activeDay.isRest ? 'Rest & Recover' : 'Today'}</p>
                        </div>
                     ...
                     <p className="text-zinc-400 text-sm mb-8 leading-relaxed font-medium bg-dark-900 p-4 rounded-xl border border-white/5">
                        {activeDay.analysis || (activeDay.isRest 
                        ...
                        <div>
                           ...
                           <div className="space-y-4">
                              {activeDay.mappedExercises.map((ex, idx) => (
                                 <div key={idx} className="flex items-center gap-4 group cursor-pointer hover:bg-white/5 p-2 -mx-2 rounded-xl transition-colors">
                                    <div className="w-16 h-16 bg-white/10 rounded-xl overflow-hidden p-1 flex items-center justify-center relative backdrop-blur-sm border border-white/5">
                                       <img 
                                          src={ex.gifUrl || "https://images.unsplash.com/photo-1541534741688-6078c6bfb5c5?auto=format&fit=crop&w=800&q=80"} 
                                          alt={ex.name}
                                    ...
                                    <div className="flex-1">
                                       <p className="font-bold text-white capitalize text-sm group-hover:text-brand-400 transition-colors">{ex.name}</p>
                                       <p className="text-xs text-zinc-500 capitalize mt-0.5">{ex.bodyPart || 'Full Body'}</p>
                                    </div>
